...
Deno.test(
  "handleCallback() rejects for non-existent OAuth session",
  async () => {
    const request = new Request("http://example.com", {
...
Deno.test(
  "handleCallback() correctly handles the callback response",
  async () => {
...
Deno.test(
  "handleCallback() correctly handles the callback response with options",
  async () => {
    const tokensBody = randomTokensBody();
    const fetchStub = stub(
      window,
